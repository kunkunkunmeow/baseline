...
import post_promotion_prediction_query
...
fh = logging.FileHandler('post_promotion_prediction.log')
...
def load_t2_from_bq(section, project_id):
    ...
    # 1 weeks worth post promotion baseline
    fcast.avg_forecast_period*1 as p_qty_bl
    ...
    total_time = round((time.time() - start_time) / 60, 1)
    logger.info("Completed loading of prediction post promotion table from Bigquery {a} mins...".format(a=total_time))
...
if __name__ == "__main__":
    ...
    if run_config == 'train' or run_config == 'train-predict':
        logger.info("Training the prediction model for the post promotion period...")
      
        # obtain input data
        logger.info("Writing historical post promotion performance data table in Bigquery...")
        post_promotion_prediction_query.promotion_prediction_(project_id, dataset_id, bl_s)

        logger.info("Loading historical post promotion performance data table from Bigquery...")
    ...
    if run_config == 'train-predict':
        ...
        logger.info("Predicting the post promotional performance for in-scope skus....")
        ...
        for i_sec in range(0, len(unique_sections)):
            ...
            if (i_sec == 0):
                pandas_gbq.to_gbq(results_df, 'prediction_results.prediction_post_promotion_results', project_id=project_id, if_exists=bl_table_config)
            else:
                pandas_gbq.to_gbq(results_df, 'prediction_results.prediction_post_promotion_results', project_id=project_id, if_exists='append')
        ...
        logger.info('Creating prediction post promotion top 20 table in Bigquery...')
        post_promotion_prediction_query.promotion_prediction_res(project_id, dataset_id)
        logger.info('Completed creating prediction post promotion top 20 table in Bigquery...')
