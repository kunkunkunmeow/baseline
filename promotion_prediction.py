...
def load_t2_from_bq(section, area, project_id):
    ...
    summary_sql = """
    WITH pr_train AS (
    SELECT distinct promo_mechanic, Promo_mechanic_en , discount_depth, discount_depth_rank
...
if __name__ == "__main__":
    ...
    if run_config == 'train-predict':
        ...
        logger.info('Creating prediction promotion table in Bigquery...')
        promotion_prediction_query.promotion_prediction_res(project_id, dataset_id)
        logger.info('Completed creating prediction promotion table in Bigquery...')
